...
def min_cost(no_of_nodes, demand, Edge, Cost, Part_Flow  ):
	...
	No_Max_Flow = [[0 for x in range(n)] for x in range(n)] #Stores number of times max-flow is run between a fixed source and fixed sink 
	...
	print "No. of times max. flow is run between fixed source and fixed sink is: ", No_Max_Flow
...
def saturate_source(s,n,Edge,Cost, node_potential,Part_Flow, curr_demand, sink,no_sat_source, Tight_Loose, Tight_Loose_No, No_Max_Flow ):
	...
	#Need to get f by min. 
		...
		maxFlow = edmonds_karp(n,s,t,MF_Capacity, MF_Flow)
